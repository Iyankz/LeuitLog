...
install_dependencies() {
    ...
    # Install Python packages using apt (preferred on Debian/Ubuntu)
    apt-get install -y -qq \
        python3-flask \
        gunicorn \
        > /dev/null 2>&1 || {
        # Fallback to pip with --break-system-packages if apt packages not available
        pip3 install --quiet --break-system-packages flask gunicorn 2>/dev/null || \
        pip3 install --quiet flask gunicorn --break-system-packages
    }
